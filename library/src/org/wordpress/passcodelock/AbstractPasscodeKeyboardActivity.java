...
public abstract class AbstractPasscodeKeyboardActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPinCodeField.setClickable(true);
        mPinCodeField.setFocusable(true);
        InputFilter filter[] = new InputFilter[1];
        filter[0] = new InputFilter.LengthFilter(PASSWORD_LENGTH);
        mPinCodeField.setFilters(filter);
    ...
    private OnClickListener defaultButtonListener = new OnClickListener() {
        ...
        public void onClick(View arg0) {
            ...
            if (mPinCodeField.length() >= PASSWORD_LENGTH) {
                onPinLockInserted();
            }
        }
    };
    ...
    protected void authenticationSucceeded() {
        if (AppLockManager.getInstance().getUnLockListener() != null) {
            AppLockManager.getInstance().getUnLockListener().onSuccess(true);
        }
        getAppLock().forcePasswordLock(false);
        CryptoUtils.init(this, mPinCodeField.getText().toString());
        setResult(RESULT_OK);
        finish();
    }
    ...
    protected void authenticationFailed() {
        ...
        Thread shake = new Thread() {
            public void run() {
                Animation shake =
                        AnimationUtils.loadAnimation(AbstractPasscodeKeyboardActivity.this, R.anim.shake);
    ...
    protected void showPasswordError() {
        Toast toast = Toast.makeText(AbstractPasscodeKeyboardActivity.this,
                getString(R.string.passcode_wrong_passcode), Toast.LENGTH_SHORT);
